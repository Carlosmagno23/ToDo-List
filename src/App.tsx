...
function App() {
  ...
  const chavesTarefasMemoria = "tarefas"
  ...
  useEffect(() => {
    if(estaCarregado) {
      localStorage.setItem(chavesTarefasMemoria, JSON.stringify(todos))
    }
  }, [todos, estaCarregado])  

  useEffect(() => {
    const tarefasdaMemoria = localStorage.getItem(chavesTarefasMemoria)
